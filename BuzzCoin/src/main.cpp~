...
int64 GetProofOfStakeReward(int64 nCoinAge, int64 nBalance, unsigned int nBits, unsigned int nTime, int nHeight)
{
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            if (nStakeReward > GetProofOfStakeReward(nCoinAge, nBalance, pindexBlock->nBits, nTime, pindexBlock->nHeight) - GetMinFee() + MIN_TX_FEE)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BuzzCoin
    bool fStrictPayToScriptHash = true; // Always active in BuzzCoin
    ...
    // ppcoin: track money supply and buzz amount info
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BuzzCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (%u,%u) %s  %08x  %s  buzz %7s  tx %"PRIszu"",
            pindex->nHeight,
            pindex->nFile,
            pindex->nBlockPos,
            block.GetHash().ToString().c_str(),
